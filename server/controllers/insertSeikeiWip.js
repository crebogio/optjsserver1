...
const insertSeikeiWip = async (req, res) => {
  const { ctrl_no,item_no,mold_no,batch_no,machine,employee,start_time} = req.body; 
  ...
  if (isEmployeeValid.length > 0) {
    if (isMachineValid.length > 0) {
          const itemEntry = await dbInsertSeikeiWip(ctrl_no,item_no, mold_no, batch_no,machine,employee,start_time);
